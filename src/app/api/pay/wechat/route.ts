...
export async function POST(request: Request) {
  try {
    ...
    // TODO: In production, integrate with WeChat Pay SDK
    // const payParams = await wechatPay.createOrder({
    //   outTradeNo: order.orderNo,
    //   totalFee: Number(order.payAmount) * 100,
    //   body: order.items.map(i => i.productName).join(', '),
    // })

    // Demo mode: return mock payment URL
    const payUrl = `/api/pay/wechat/demo?orderId=${order.id}&paymentId=${payment.id}`;
